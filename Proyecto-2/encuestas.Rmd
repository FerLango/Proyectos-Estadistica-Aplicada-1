...
sup <- sup %>% 
    add_column(abi = c(1,sup$cum[1:length(sup$cum)-1])) %>%
    add_row(tiempo=0,cum=1) %>%
    arrange(tiempo)
sup %>%
    ggplot() +
    geom_point(aes(x=tiempo, y=cum),pch=19, color="blue",size=2) +
    geom_point(aes(x=tiempo, y=abi),pch=1, color="red",size=2)
```
