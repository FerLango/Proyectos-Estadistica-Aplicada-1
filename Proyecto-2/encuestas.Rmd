...
temp <- pacientes_pob %>%
        ...
        tally() %>%
        mutate(p = n/N) %>%
        mutate(cum = 1 - cumsum(p), abi = NULL)

time1 <- c(0,temp$tiempo)
prob1 <- c(1,1,temp$cum[1:length(temp$cum)-1])

temp <- temp %>% 
    add_column(abi = c(1,temp$cum[1:length(temp$cum)-1])) %>%
    add_row(tiempo=0,cum=1) %>%
    arrange(tiempo)

temp %>%
    ggplot() +
    geom_point(aes(x=tiempo, y=cum),pch=19, color="blue",size=10) +
    geom_point(aes(x=tiempo, y=abi),pch=1, color="red",size=10)
```
