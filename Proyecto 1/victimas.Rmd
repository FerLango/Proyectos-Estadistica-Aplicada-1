...
ggplotly(heat, dynamicTicks = TRUE)
```
...
ggplotly(box, dynamicTicks = TRUE)
```
...
Podemos apreciar que la cantidad de delitos disminuyeron del 2019 al 2020 y que la mayoría de las personas que denuncian tienen calidad jurídica de `r victimas %>% group_by(CalidadJuridica) %>% tally() %>% arrange(desc(n)) %>% select(CalidadJuridica) %>% head(1)`.
